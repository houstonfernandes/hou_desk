...
/* WEBPACK VAR INJECTION */(function($) {__webpack_require__(7); //@todo sem o validation na compilacao esta dando erro 20/09/17
...
window.upload = function () {
    ...
    $.ajax({
        ...
        success: function success(data) {
            //console.log(files);
            var file = data.file;
            var liFile = $('<li>').append(file.fileName);

            var aFileThumb = $('<img>').attr({ 'src': file.url_thumbnail }).css({ 'with': '80px', 'max-height': '400px' }); //.addClass('rounded-circle');
            var aFile = $('<img>').attr({ 'src': file.url }).css({ 'with': '80px', 'max-height': '400px' }); //.addClass('rounded-circle');
            liFile.append(aFileThumb);
            liFile.append(aFile);
            $('#arquivos_enviados').append(liFile);
            $('#panel_arquivos_enviados').show();
            var msg = 'Arquivo: ' + file.fileName + ' enviado com sucesso.';
            exibirAlerta(msg, 'success');
            limparForm();
            //$('#progress-bar').hide();

            var files = data.document_files;
            if (files != null) {
        ...
        error: function error(jqXhr) {
            ...
            if (jqXhr.status === 422) {
                ...
                var _parsedJson = jqXhr.responseJSON;
                //console.log(parsedJson)
                var errorString = '';
                $.each(_parsedJson.errors, function (key, value) {
                    errorString += '<ul>' + key;
    ...
    atualizarCheckboxArquivos = function atualizarCheckboxArquivos(documentFiles) {
        //console.log('em atualizar checkbox');
        //console.log(documentFiles);

        var div = $('<div class="checkbox">');
        var label = $('<label>');
        var checkbox = $('<input type="checkbox" name="arquivo_id[]">');

        var divDocumentFiles = $('#divDocumentFiles');
        divDocumentFiles.empty();
        $.each(documentFiles, function (i, arquivo) {
            //console.log('nome =' +arquivo.nome +arquivo.id);
            var divDoc = div.clone();
            var labelDoc = label.clone();
            var checkboxDoc = checkbox.clone().attr('value', arquivo.id);
 ...
 */
window.jquery_validation = __webpack_require__(5);
__webpack_require__(8);
__webpack_require__(9); //jquery-validation
...
$(function () {
	...
	jQuery.validator.addMethod("semAcento", function (value, element) {
		return this.optional(element) || /^([a-zA-Z0-9]|[ ])+$/.test(value);
	}, "Utilize somente letras ou números.");

	jQuery.validator.addMethod("semAcentoEspaco", function (value, element) {
		return this.optional(element) || /^([a-zA-Z0-9])+$/.test(value);
