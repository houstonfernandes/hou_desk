...
@section('content')
    ...
    <form method="post" action="{{route('admin.clientes.store')}}" name='form' >
    {{ csrf_field() }}
    
      <div class="form-group">
        <label for="nome">Nome</label>
        <input type="text" class="form-control" id="nome" name="nome" value="{{old('nome')}}" placeholder="nome do cliente" maxlength="100" required>
      </div>
      
  		<div class="form-inline">
    		<input class="form-check-input" type="radio" name="tipo_cliente" id="tipo_cliente_fisica" value="0" {{old('tipo_cliente',0)==0 ?'checked':''}}>
      		<label class="form-check-label" for="tipo_cliente_fisica">Pessoa Física</label>
      		<input class="form-check-input" type="radio" name="tipo_cliente" id="tipo_cliente_juridica" value="1" {{old('tipo_cliente')==1 ?'checked':''}} >
...
@push('js')
    ...
    <script src="{{asset('js/admin/clientes_create.js')}}"></script>
@endpush